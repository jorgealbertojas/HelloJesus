package com.softjads.jorge.hellojesus.util.download;
...
import static com.softjads.jorge.hellojesus.util.download.Utility.BASE_STORAGE;
import static com.softjads.jorge.hellojesus.util.download.Utility.BASE_URL_IMAGE;
import static com.softjads.jorge.hellojesus.util.download.Utility.EXTRA_DOWNLOAD;
import static com.softjads.jorge.hellojesus.util.download.Utility.EXTRA_POSITION;
import static com.softjads.jorge.hellojesus.util.download.Utility.EXTRA_POSITION_NUMBER;
import static com.softjads.jorge.hellojesus.util.download.Utility.MESSAGE_PROGRESS;
import static com.softjads.jorge.hellojesus.util.download.Utility.TAG_INFORMATION;
...
public class DownloadService extends IntentService {
    ...
    private void initDownload(String fileName, String position) {
        ...
        InterfaceDownload retrofitInterface = retrofit.create(InterfaceDownload.class);
                ...
                Call<ResponseBody> request = retrofitInterface.downloadFile(BASE_URL_IMAGE +  fileName);
       // Call<ResponseBody> request = retrofitInterface.downloadFile(BASE_URL_IMAGE_COMPLEMENT +  "Mattew_2" + ".mp3?alt=media&token=0ec42699-b932-4cfd-9d4c-fe3cd8042aa9");
        try {

            downloadFile(request.execute().body(), Common.returnFile(fileName) , position);
    ...
    private void downloadFile(ResponseBody body, String fileName, String position) throws IOException {
        ...
        File outputFile = new File(BASE_STORAGE, fileName);
